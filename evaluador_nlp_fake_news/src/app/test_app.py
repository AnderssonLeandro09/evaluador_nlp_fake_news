...
from .services.evaluator import evaluation_service as service
from .services.evaluator import EvaluationService
...
class TestService:
    ...
    async def test_evaluate_text(self):
        ...
        with patch.object(service_instance, '_infer_beto', new_callable=AsyncMock) as mock_beto, \
             patch.object(service_instance, '_infer_mbert', new_callable=AsyncMock) as mock_mbert:
            
            mock_beto.return_value = (1, 0.1)
            mock_mbert.return_value = (0, 0.15)
            
            result = await service_instance.evaluate("Test text", 1)
            
            # The result is a tuple (BETO_res, mBERT_res)
            beto_res, mbert_res = result
            
            assert beto_res.prediction == 1
            assert mbert_res.prediction == 0
            assert beto_res.time_ms == 0.1
            assert mbert_res.time_ms == 0.15
            assert beto_res.accuracy == 1.0
            assert mbert_res.accuracy == 0.0
